...
export const FINDER_TREE: Partial<Record<MuscleGroup, FinderOption[]>> = {
  chest: [
    { key: 'flat', labelKo: '평평하게 밀기', labelEn: 'Flat press', names: ['바벨 벤치프레스', '덤벨 벤치프레스', '체스트 프레스 머신', '스미스 벤치프레스', '푸시업', '해머스트렝스 체스트 프레스', '라잉 체스트 프레스 머신', '컨버징 체스트 프레스 머신', '원암 체스트 프레스 머신', '시티드 케이블 체스트 프레스', '스탠딩 케이블 체스트 프레스', '라잉 케이블 체스트 프레스', '라슨 프레스', '피트 업 벤치프레스', '와이드 그립 푸시업', '데피싯 푸시업'] }, // 갭 이관 @plm SRS-047
    { key: 'incline', labelKo: '위로 기울여 밀기(인클라인)', labelEn: 'Incline press', names: ['인클라인 프레스 (바벨)', '인클라인 프레스 (덤벨)', '인클라인 프레스 (머신)', '인클라인 프레스 (스미스)', '인클라인 푸시업'] },
    { key: 'decline', labelKo: '아래로 밀기(디클라인·딥스)', labelEn: 'Decline / dips', names: ['디클라인 프레스 (바벨)', '디클라인 프레스 (덤벨)', '디클라인 프레스 (스미스)', '딥스', '디클라인 체스트 프레스 머신'] }, // 갭 이관 @plm SRS-047
  ...
  back: [
    ...
    { key: 'row', labelKo: '앞으로 당기기(로우)', labelEn: 'Row', names: ['바벨 로우', '펜들레이 로우', '티바 로우', '덤벨 로우', '시티드 케이블 로우', '머신 로우', '로우 (스미스)', '하이 로우', '로우 로우', '체스트 서포티드 로우', '체스트 서포티드 로우 (덤벨)', '체스트 서포티드 티바 로우', '인버티드 로우'] }, // 갭 이관 @plm SRS-047
  ...
  shoulders: [
    { key: 'press', labelKo: '위로 밀기(프레스)', labelEn: 'Press', names: ['오버헤드 프레스', '밀리터리 프레스', '덤벨 숄더 프레스', '아놀드 프레스', '숄더 프레스 머신', '숄더 프레스 (스미스)', '랜드마인 프레스'] },
    { key: 'lateral', labelKo: '옆으로 들기(레터럴)', labelEn: 'Lateral raise', names: ['덤벨 레터럴 레이즈', '케이블 레터럴 레이즈', '머신 레터럴 레이즈', '업라이트 로우 (바벨)', '업라이트 로우 (케이블)', '업라이트 로우 (덤벨)'] },
    { key: 'front', labelKo: '앞으로 들기(프론트)', labelEn: 'Front raise', names: ['프론트 레이즈 (덤벨)', '프론트 레이즈 (케이블)', '프론트 레이즈 (바벨)'] },
  ...
  triceps: [
    ...
    { key: 'extension', labelKo: '펴는 동작(익스텐션·푸시다운)', labelEn: 'Extension', names: ['트라이셉스 푸시다운', '로프 푸시다운', '오버헤드 트라이셉스 익스텐션', '케이블 오버헤드 익스텐션', '스컬 크러셔 (바벨)', '스컬 크러셔 (덤벨)', '트라이셉스 킥백 (덤벨)', '트라이셉스 킥백 (케이블)'] },
  ],
  quads: [
    { key: 'squat', labelKo: '앉았다 일어서기(스쿼트)', labelEn: 'Squat', names: ['바벨 스쿼트', '프론트 스쿼트', '하이바 스쿼트', '핵 스쿼트', '고블릿 스쿼트', '스미스 스쿼트', '시시 스쿼트', '펜듈럼 스쿼트', '벨트 스쿼트'] },
    { key: 'lunge', labelKo: '한 다리씩(런지·스텝업)', labelEn: 'Lunge / step-up', names: ['런지 (덤벨)', '런지 (바벨)', '런지 (스미스)', '워킹 런지', '불가리안 스플릿 스쿼트 (덤벨)', '불가리안 스플릿 스쿼트 (바벨)', '불가리안 스플릿 스쿼트 (스미스)', '스텝업'] },
  ...
  glutes: [
    { key: 'thrust', labelKo: '밀어올리기(힙 쓰러스트·브릿지)', labelEn: 'Thrust / bridge', names: ['힙 쓰러스트 (바벨)', '힙 쓰러스트 (머신)', '글루트 브릿지', '스미스 힙 쓰러스트'] },
  ...
  abs: [
    { key: 'crunch', labelKo: '윗몸 말기(크런치)', labelEn: 'Crunch', names: ['크런치', '케이블 크런치', '싯업', '디클라인 싯업', '앱 크런치 머신', '바이시클 크런치'] }, // 갭 이관 @plm SRS-047
