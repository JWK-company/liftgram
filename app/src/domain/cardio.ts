...
// 기구마다 기록할 정보가 다르다: 러닝머신=경사·속도, 실내 사이클·천국의 계단=단계 등.
// incline(경사 %)과 level(단계)은 상호배타(기구는 둘 중 하나). speed(속도 km/h)는 러닝머신 전용.
// 실외 러닝·걷기는 경사·속도 미기록(시간·거리만). 미매핑(커스텀)은 기본 [시간·거리].
export type CardioMetric = 'duration' | 'distance' | 'incline' | 'level' | 'speed';
...
export interface CardioTarget {
  ...
  speed?: number | null; // 속도 km/h (러닝머신)
}

// nameEn 기준 종목별 지표. 새 종목 추가 시 여기에 등록.
const CARDIO_METRICS_BY_NAME_EN: Record<string, CardioMetric[]> = {
  'Treadmill Running': ['duration', 'distance', 'incline', 'speed'], // 러닝머신 — 경사·속도 기록
  Running: ['duration', 'distance'], // 실외 러닝 — 경사·속도 미기록
  Walking: ['duration', 'distance'], // 걷기 — 경사·속도 미기록
...
// 속도(km/h) 입력 → 저장 number. 빈/0/음수/NaN = null. 소수1자리(러닝머신 표시 정밀도).
export function inputToSpeed(text: string): number | null {
  const n = parseFloat(text.replace(',', '.'));
  return Number.isNaN(n) || n <= 0 ? null : Math.round(n * 10) / 10;
}
