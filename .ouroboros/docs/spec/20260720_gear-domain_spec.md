...
- **전제 결정**: ADR-027 (D2 제휴 파라미터 없이 출발 · D5 브랜드 조건부 태그(사용자 확인 필수) · D6 고지 라벨 강제 · D7 source 보존)
- **autorun**: RM-016 큐 1/6
- **개정**: rev4 (2026-07-20) — ADR-027 D5 개정 반영(브랜드 조건부 허용). **구현 완료 커밋 7e818fa.**
  개정 D5의 허용 3경로 = ①사용자 직접 입력(Phase 0 범위) ②자동 감지 제안 + 사용자 확인(Phase 1, 재측정 통과 후) ③카테고리 8종 폴백.
  유일한 금지선 = **사용자가 확인하지 않은 자동 감지 결과의 게시·표시**. 확인 여부는 별도 boolean이 아니라 **`brand`가 저장돼 있다는 사실 자체**가 증명한다(미확인 제안은 저장 경로 부재 → 금지선이 데이터 모델 수준에서 구조적으로 막힌다).
  근거 = 블라인드 14장 실측(브랜드 오단정 0건 · high 신뢰 9건 전건 적중 · 카테고리 14/14 · 함정 오탐 0/2 · 로고 미노출 시 기권 2건), 보고서 `.ouroboros/docs/research/20260720_gear-brand-detection-accuracy_research.md`.
  **한계 병기(반드시 함께 읽을 것)**: 12/14가 브랜드 공식몰 커머셜 사진이라 **낙관 편향 상한선**이다 · 채점 모델 Opus급 · 표본 14장 · 국내 브랜드 0종.
  변경 = `GearTag.brand`·`brandSource` 신설 · `MAX_GEAR_BRAND_LEN` · `resolveGearLink`가 `GearTag`도 수용 · 테스트 20→21그룹(T8 뒤집음 · T21 신설). **정리 A·B, 링크 조작 금지, 고지 게이트, 오버레이 금지, 수익 환원 영구 배제는 전부 그대로다.**
...
export interface GearTag {
  ...
  // rev4 신설(ADR-027 D5 개정판). 저장돼 있다는 것 자체가 사용자 확인 통과를 뜻한다 — 미확인 자동 제안은
  // 게시 전 확인 UI 안에만 존재하고 저장 경로가 없으므로 별도 confirmed 플래그가 필요 없다.
  readonly brand?: string;
  // 그 brand 문자열의 원천. 위 source(태그 자체의 원천)와 **별개 축**이다 — 둘을 합치면
  // (확인된 브랜드를 담으려고 태그 전체를 source:'auto'로 바꾸는 식) D7 기반 성과 분리 측정이 오염된다.
  readonly brandSource?: GearSource;
  readonly note?: string;   // Phase 0 비입력·비렌더 — 보존 전용(§note 정책)
}
// 불변식: brand ⟺ brandSource (한쪽만 있으면 정규화가 제거한다). Phase 0의 brandSource는 항상 'user'.

// 타입 주석 없이 선언 — 리터럴 타입(8 / 30 / 40 / 200) 보존
export const MAX_GEAR_TAGS = GEAR_CATEGORIES.length;  // 8
export const MAX_GEAR_NOTE_LEN = 30;
export const MAX_GEAR_BRAND_LEN = 40;                 // note의 30과 별개 상수(§상한값 확정)
...
export interface GearAffiliateConfig {
  ...
  // 운영자가 특정 SKU를 확정하는 것이어서 ADR-027 D5 개정판의 허용 3경로(사용자 직접 입력 /
  // 사용자가 확인한 자동 제안 / 카테고리 폴백) 어디에도 속하지 않는다(§links 정책). 코드는 이를 검증할 수 없다.
...
// rev4: 첫 인자가 GearCategory | GearTag 다. 태그를 넘기면 확인된 brand 가 검색어에 반영된다.
// 카테고리만 넘기는 기존 호출은 문자 단위로 동일한 URL을 돌려준다(회귀 없음 — T21-(1)이 고정).
...
// function gearSearchQuery(c: GearCategory, brand?: string): string;   // brand 는 앞에 붙인다: 'SBD 리프팅 벨트'
// function coupangSearchUrl(c: GearCategory, brand?: string): string;  // brand 는 q 값에만 — 쿼리 키는 여전히 q 하나
// function isAllowedAffiliateUrl(u: unknown): u is string;
// function hasAnyAffiliateLink(cfg: unknown): boolean;
// function sanitizeBrand(v: unknown): string | undefined;              // note 와 동일 규칙, 상한만 MAX_GEAR_BRAND_LEN
```

- **brand 반영은 검색 URL에만 적용된다.** 딥링크는 brand 유무와 무관하게 **문자 단위 무가공 반환**이다(운영정책 4.1 1) 링크 조작 금지). T21-(3)이 고정한다.
- 검색어는 `` `${brand} ${GEAR_SEARCH_QUERY[c]}` `` 형태로 **카테고리어를 남긴다** — 브랜드만으로 검색하면 그 브랜드의 전 상품군이 나와
  운영정책 4.1 6) *"검색 키워드가 관련성이 지나치게 떨어지는 상품과 연계"* 조항에 가까워진다.
- 쿼리 키가 늘지 않으므로 **정리 B(비활성 시 추적 식별자 0개)는 그대로 성립**한다.
...
| `MAX_GEAR_BRAND_LEN` | 40 (rev4) | **note의 30과 공유하지 않는 별개 상수**다 — 용도가 다르고(brand는 칩 라벨에 렌더되고 검색어 q 값에 실린다) 상한 조정 시점도 다르다. 40은 국내외 브랜드명 + 모델명 조합(`SBD 13mm Lever Belt` 류)이 잘리지 않는 폭. 공유 상수로 묶으면 한쪽 사정으로 조정할 때 다른 쪽이 조용히 끌려간다 |
...
### §links 정책 — 상품 상세 딥링크는 개정 D5의 허용 3경로 밖이다 (rev3 신설 · rev4 근거 정정)

**rev4 정정**: 이 소절의 근거는 더 이상 "브랜드·모델 미특정"이 아니다(개정 D5가 브랜드를 조건부 허용했다).
정확한 근거는 **주체**다 — 상품 상세 딥링크를 넣는 것은 **운영자가 특정 SKU를 확정하는 행위**이고,
개정 D5의 허용 3경로(①사용자 직접 입력 ②사용자가 확인한 자동 감지 제안 ③카테고리 폴백) **어디에도 속하지 않는다**.
세 경로 모두 특정의 주체가 **그 게시물의 사용자**인데, 링크 목적지는 사용자가 관여하지 않는 서버 주입값이기 때문이다.
사용자가 `brand: 'SBD'`를 직접 입력한 게시물에서도 마찬가지다 — 그 브랜드는 **검색어**에 반영될 뿐이고(§공개 API 설계),
운영자가 고른 상품 상세로 보내는 것은 별개의 확정이다.
...
(a) 브랜드·모델 특정의 **주체가 사용자여야 한다**는 ADR-027 D5 개정판의 전제가 링크 계층에서 깨지고(운영자가 사용자 대신 SKU를 확정 — 허용 3경로 밖),
...
### note 정책 — `brand` 와 분리된 자유 텍스트 축 (rev4 재서술)

**rev4 정정**: rev3의 이 소절은 *"note에 브랜드가 들어가면 D5가 막으려던 표면이 복원된다"*를 근거로 삼았다.
개정 D5가 브랜드를 조건부 허용하고 **SRS-040이 브랜드 칩 라벨 표시를 의무화**했으므로 그 근거는 그대로 두면 정면 충돌한다.
충돌은 `note`와 `brand`를 **분리**해 해소한다 — 둘은 같은 것의 두 표현이 아니라 **별개 축**이다.

| | `brand` (rev4 신설) | `note` |
|---|---|---|
| 성격 | **구조화 필드** — 값의 의미가 스키마로 고정 | **자유 텍스트** — 무엇이든 들어올 수 있음 |
| 확인 | 저장 = 사용자 확인 통과(미확인 제안은 저장 경로 부재) | 확인 개념 자체가 없음 |
| 표시 | SRS-040이 **칩 라벨에 병기**(의무) | Phase 0 **비렌더** |
| 검색어 | q 값에 반영(`SBD 리프팅 벨트`) | **반영 안 함** |
| 상한 | `MAX_GEAR_BRAND_LEN` (40) | `MAX_GEAR_NOTE_LEN` (30) |

즉 브랜드를 담고 싶으면 **`brand` 필드에 담는 것이 유일한 정답**이고, note를 브랜드 우회 경로로 쓰지 않는다.
`note`는 SRS-037 수용 기준 2("태그별 메모 길이 상한 강제")가 요구하는 필드라 타입에서 제거하지 않는다.
소유자 부재를 방치하지 않기 위해 Phase 0 규약을 여기서 확정한다.

1. **입력 주체 없음** — SRS-038 GearTagPicker는 note를 입력받지 않는다. Phase 0에서 note는 항상 `undefined`이며, 정규화는 외부(조작된 클라이언트·손상 데이터)에서 들어온 값을 **보존·정제만** 한다. (반면 `brand`는 Phase 0에서 **입력 주체가 있다** — GearTagPicker·MyGearScreen이 사용자 입력을 받는다.)
2. **비렌더** — SRS-040 GearChips는 note를 렌더하지 않는다(브랜드 병기는 `brand` 필드가 담당한다). 근거는 브랜드 회피가 아니라 **자유 텍스트의 무검증성**이다 — 30자 상한은 길이 제한일 뿐 내용 제한이 아니라 욕설·타인 식별정보·URL·광고 문구가 무엇이든 들어올 수 있고, 입력 UI가 없어 **정상 경로로는 값이 생기지도 않는다**(값이 있다면 조작·손상 데이터라는 뜻). UGC의 법적 책임은 전부 회원사(우리)에게 있다(이용약관 제11조·제20조 4항).
3. **링크 무관** — note는 검색어·링크 조립 어디에도 반영되지 않는다(테스트로 고정 — 같은 카테고리는 note가 달라도 항상 동일 URL). **`brand`는 반영된다** — 이것이 두 필드의 가장 뚜렷한 동작 차이다.
4. 렌더가 필요해지는 시점에 자유 텍스트 억제 규칙(금칙어·URL 링크화 금지)을 함께 결정한다. **Phase 0에서는 도입하지 않는다** — 입력 경로가 없어 방어할 대상이 없다. (rev3이 여기 적었던 "브랜드 억제 규칙(sanitize)"은 **삭제한다** — 개정 D5에서 브랜드 억제는 더 이상 목표가 아니고, 라틴 토큰 거부 류 규칙은 오탐도 컸다.)
...
- [ ] `app/src/domain/__tests__/gear.test.ts` 신규 — node:test + assert/strict. §필수 테스트의 **21개 그룹 전건**(rev4 — T21 추가). 모든 `test()` 이름은 `T{n} ` 접두로 시작
...
0. **출력은 항상 `{ category, source }`(+조건부 `brand`·`brandSource`·`note`) 리터럴로 새로 구성한다.** 입력 객체를 반환하거나 스프레드하지 않는다 — 그 외 모든 키(`url`·거대 문자열·데이터 키로 실린 `__proto__` 등)는 폐기된다. 입력 배열·입력 객체를 변형하지 않는다.
   - **rev4**: `brand`·`brandSource`는 화이트리스트 **안**으로 편입됐다(rev3까지는 폐기 대상이었고 T8이 그 폐기를 정답으로 고정하고 있었다 — 개정 D5로 뒤집혔다). 규칙은 셋뿐이다:
     ① `brand`는 `note`와 **같은 정제 파이프라인**을 상한만 `MAX_GEAR_BRAND_LEN`으로 바꿔 통과시킨다(제어문자·고립 서로게이트 제거 → 공백 접기 → trim → 코드포인트 절단 → 재trim → 빈 문자열이면 키 제거).
     ② `brandSource`는 `'auto'`가 아니면 전부 `'user'`로 수렴한다. **태그의 `source`와 별개 축이라 서로를 건드리지 않는다.**
     ③ **불변식 `brand` ⟺ `brandSource`** — 정제 후 `brand`가 없으면 `brandSource`도 키째 제거하고, `brand`만 있으면 `brandSource`를 `'user'`로 채운다. 중복 병합 시에도 두 키는 **반드시 쌍으로** 옮긴다(§규칙 6c).
   - **허용 구현 형태(rev3 명시 · rev4 확장)**: `note !== undefined ? { category, source, note } : { category, source }` 2분기 리터럴,
     또는 `{ category, source, ...(brand !== undefined ? { brand, brandSource } : {}), ...(note !== undefined ? { note } : {}) }`.
...
6. 같은 `category`가 중복되면 → **순서 무관 우선순위 병합**.
   ...
   (c) 이긴 쪽에 `note`가 없고 진 쪽에 있으면 `note`를 승계한다(이름값 하는 "병합"). **rev4**: `brand`도 같은 방식으로 승계하되 **`brandSource`와 반드시 쌍으로 옮긴다** — 값만 오고 출처가 갈리면 "확인된 브랜드"의 추적성이 깨져 개정 D5의 금지선(미확인 자동 감지 결과의 표시) 판별 근거가 사라진다. **병합 결과도 반드시 규칙 0의 형태로 새 리터럴을 재구성**한다(기존 객체 변형 금지 — `readonly` 위반).
...
## 필수 테스트 (gear.test.ts — 21그룹 전건, 누락 시 미완료)
...
- **모든 `test()` 이름은 `T{n} ` 접두로 시작**한다 — 예: `test('T7 화이트리스트: Belt·__proto__·미지 문자열 폐기', …)`.
  러너는 그룹 개념을 모르고 리포 관례상 테스트명이 한국어 서술형이라, 접두 없이는 "T1~T21 전 그룹 포함"을 기계적으로 확인할 수단이 없다(`npm test 2>&1 | grep -oE 'T[0-9]+ ' | sort -u | wc -l`로 확인 가능해진다).
...
| T8 | 출력 형태 (**rev4에서 뒤집음**) | **rev3까지 이 그룹은 `brand:'X'` 폐기를 정답으로 고정하고 있었다 — 개정 D5(실측: 브랜드 오단정 0건)로 근거가 반박돼 `brand`·`brandSource`는 이제 화이트리스트 '안'이다. 폐기 고정 → 보존 검증으로 전환한다.** `{category:'belt',source:'user',extra:1,brand:'SBD',url:'https://x'}` → `Object.keys(r[0])`가 `['category','source','brand','brandSource']` · `r[0].brand === 'SBD'` · `r[0].brandSource === 'user'`(부재 시 수렴) · 화이트리스트 **밖** 키(`extra`·`url`)는 **여전히 폐기** · `Object.getPrototypeOf(r[0]) === Object.prototype` · JSON 리터럴 `__proto__` 키가 결과에 없고 전역 프로토타입 미오염 |
...
| **T21** | **브랜드 (rev4 신설 — ADR-027 D5 개정판)** | 10항목 전건. ① **회귀 없음**: 8종 전수로 `resolveGearLink(c, …)`와 `resolveGearLink({category:c, source:'user'}, …)`의 URL이 **문자 단위 동일** ② **검색어 반영**: `{belt, brand:'SBD', brandSource:'user'}` → `kind==='search'` · `[...searchParams.keys()]`가 정확히 `['q']`(추적 파라미터 0개 유지) · `q === 'SBD 리프팅 벨트'` · `q !== 'SBD'`(카테고리어를 지우지 않는다 — 관련성 조항) ③ **딥링크 무가공**: brand 유무와 무관하게 `url === DEEP`(링크 조작 금지) ④ **고지 게이트 불변**: brand가 있어도 `disclosureRendered:false`면 `blocked:'disclosure-missing'` ⑤ **`brand` ⟺ `brandSource` 불변식**: brand 없는 `brandSource`는 키째 제거 · brandSource 없는 brand는 `'user'`로 수렴 ⑥ **별개 축**: `{source:'user', brandSource:'auto'}`가 그대로 보존(확인을 거쳐도 auto 원천 표시를 지우지 않는다 — D7 성과 분리 측정) ⑦ **brand 정제**(note와 같은 정제 파이프라인, 상한만 다름): `'A'.repeat(60)` → 코드포인트 길이 `MAX_GEAR_BRAND_LEN` · `42`·`null`·`{}`·`[]`·`'   '`·`'\u0000\u0000'` → 키째 제거 · `' S\u0000B D '` → `'S B D'` ⑧ **병합 승계**: 진 쪽의 `brand`를 이긴 쪽이 물려받되 `brandSource`도 **쌍으로** 함께 온다(값과 출처가 갈리면 추적성이 깨진다) ⑨ **멱등**: `brand:'  SBD  '`·`brandSource:'auto'` 포함 입력에 대해 `normalize(normalize(x))` deepEqual `normalize(x)` ⑩ **미지 카테고리 태그도 차단**: `{category:'dumbbell', brand:'X'}`(캐스팅 우회) → `{ok:false, blocked:'unknown-category'}` — 태그 경로에서도 1단계가 brand 처리보다 먼저다 |
...
- 기준선: `npm test` **104 pass / 0 fail**(rev4 · brand 반영 후), `npm run typecheck` clean. 새 실패는 전부 이번 변경 탓
...
- `cd app && npm test` → **0 fail**, 그리고 `gear.test.ts`가 §필수 테스트의 **T1~T21 전 그룹을 포함**(기준선 83 + 21 = **104 pass**, rev4 구현 완료 시점 실측치). 그룹 누락은 미완료로 판정한다
- 그룹 커버리지 기계 확인: `npm test 2>&1 | grep -oE 'T[0-9]+ ' | sort -u | wc -l` 이 **21**
- 정리 A·B가 T15·T16으로 고정되어 있다 — 특히 `{enabled:false, links:{belt:DEEP}}` 조합이 `disclosure-missing`으로 차단되고, 고지 렌더 시에도 딥링크가 **아닌** 검색 URL을 돌려준다
- SRS-037 수용 기준 6개 대응:
  ① → T1·T3·T4 ② → T5~T8·T11·T12·T19 (단 "개수 상한"은 **도달 불가 — T12의 경계 보존으로 대체 검증**. §규칙 7) ③ → T9·T10 ④ → T13·T14·**T16·T17·T21**(무가공 반환·호스트 폴백·brand의 q 값 한정 반영 — 단, AC4 후단은 §개정 필요 대상) ⑤ → T15·T16 (트리거는 `enabled` 또는 딥링크 존재로 개정 필요) ⑥ → 전체
