...
## 6. 재판정 — 변형 3건 완료 처리 (2026-07-20, 소유자 판단)

제품 소유자 판단으로 **#21(기구 통일·브랜드 전환) · #19(그립 정보) · #26(원암/투암)** 을 [부분] → **[완료]** 로 재판정했다. BS-002 상태 요약은 완료 12·부분 8 → **완료 15·부분 5**.

근거: 세 항목 모두 **기능 요구 본체는 구현 완료**다 — 기구 전환 UI·기구별 버킷 분리(#21), 세트별 그립 선택·저장·수행화면 표시(#19), 세트별 투암↔원암 전환 기록 + 완료 상세 태그(#26). 원문 요구("메모나 옵션으로 기록 가능하게", "브랜드 바꾸듯 전환")를 기준으로 하면 충족이며, §1 감사가 [부분]으로 잡았던 것은 **표시 계층 보강분**이었다.

감사 판정(§1·§4)은 그 시점의 기계적 대조 기록이므로 **수정하지 않는다**. 남은 보강분은 삭제하지 않고 BS-002 본문의 "후속 개선 (별건)" 절로 이관해 추적을 유지했다:

| 이관 항목 | 실체 (직접 확인) |
|---|---|
| 그립 조회면 노출 (#19 파생) | `getPreviousExerciseSets` 반환 매핑(`workoutRepository.ts:316-327`)에 `grip` 없음 · `WorkoutDetailScreen.tsx:141`은 `arm` 태그만 렌더 |
| 원암 세트 배지 (#26 파생) | 진행 중 목록에 원암 구분 배지 없음(펼쳐야 확인). 완료 상세에는 태그 노출됨 |
| 카탈로그 물리 병합 확대 (#21 파생) | `EXERCISE_CONSOLIDATIONS`(`workoutRepository.ts:123`)에 `'인클라인 프레스'` 1건만 등록 |

즉 **"항목은 닫고, 남은 보강은 별건으로 계속 추적"** 이다 — 잔여가 없다고 기록하지는 않았다.
