...
- **SRS-032 자세 사진·설명** — 이름만으로 모르는 종목에 시작/끝 사진 교차 시연(움짤 효과) + 한국어 자세설명 + 출처 크레딧. 소스=free-exercise-db(퍼블릭도메인 데이터·CC-BY-SA 이미지), jsDelivr CDN 서빙(서버·스키마 변경 0, resolveMediaUrl 절대URL 통과 재사용). 129/144 종목 매핑(LLM 다중에이전트 정확매칭)+한국어 번역 정적 동봉. Status=Draft.
