...
- **미배포 픽스(원격 세션·2026-07-28)**: 휴식 알람 오디오 포커스 정책 — keep-alive `<audio>`+MediaSession을 iOS 한정으로 축소(utils/sound.ts). Android에서 세트 완료 후 유튜브 뮤직 등 타 앱 덕킹 제거(근무음 osc+예약+rearm/flush만으로 동작·잠금 카드 포기). tsc 0·137테스트 ✓ · **Android 잠금 알람 실기기 확인 필요** · 배포 시 v0.12.1(patch)
