...
- **검증**: `nest build` 0에러 · 서버 E2E(임시 PG): health/signup/login/me + sync(WatermelonDB 형식 create→pull→update→pull→delete)/무토큰401 전부 동작. 데이터 Postgres SyncRecord 저장 확인.
- **앱↔서버 동기 연동 (2026-06-30)**: 서버 sync를 WatermelonDB 프로토콜로 개편(pull→`{changes,timestamp}`·비삭제 전부 updated·push upsert). 앱 `src/sync/`(tokenStore 플랫폼분기·serverApi JWT·`synchronize()` sendCreatedAsUpdated) + ProfileTab `ServerSyncCard`(로그인/가입·지금 동기). 앱 typecheck·웹export·잔여한글0 PASS. (라이브 앱→서버 E2E·pgAdmin 확인은 사용자.)
- **후속**: refresh 토큰 회전 · 매니지드 인증(ADR-018) · 시드(exercises) 멀티기기 dedup · social(SAD-011)/media(SAD-012)/payments(SAD-013) 모듈. 실행: `server/README.md`.
