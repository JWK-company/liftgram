...
.PHONY: setup plugins plugin-install marketplaces ouroboros env notion guide check deps help channels channels-prompt

setup: deps marketplaces plugins plugin-install ouroboros env channels-prompt ## full install (MCP + planning plugin + env)
	@echo ""
	@[ -d .githooks ] && git config core.hooksPath .githooks 2>/dev/null && echo "  [OK] 시크릿 pre-commit hook 활성(ONB-14)" || true
	@echo "[OK] setup done."
	@echo "   - MCP plugins (chrome/context7/serena) + plan & code & plm-hub & plm-channel plugins installed"
	@echo "   - 채널(웹 [Sync]→세션): 위에서 Y 선택 시 설치됨 / 아니면 'make channels' 후 claude --channels plugin:plm-channel@jwk-platform"
...
plugin-install: ## install plan+code+plm-hub+plm-channel — project scope(프로젝트별 최신·user scope 오버라이드)
	...
	@echo "  채널: 'make channels' 후 claude --channels plugin:$(CHAN_PLUGIN)@$(MP)"
...
channels-policy: ## [조직 관리형 전용] managed-settings allowlist 등록 (sudo 1회)
	...
	@echo "  다음: /plm-hub:link <project> (토큰) → claude --channels plugin:$(CHAN_PLUGIN)@$(MP)"
...
env: ## ensure Ouroboros .env (온보딩 설치 스크립트/installer가 채움; 레거시 스크립트는 선택)
	...
	@if grep -qE '^PLM_API_TOKEN=.+' "$(ENV_FILE)" 2>/dev/null; then \
		...
		touch "$(ENV_FILE)"; \
		echo "  [-] .env 비움 — PLM은 /plm-hub:link가 토큰 자동발급, ouroboros는 .mcp.json OAuth라 .env 불필요(ONB-10)"; \
	fi
...
check: ## show install status
	@echo "=== plugins ==="
	@claude plugin list 2>/dev/null | grep -iE "plm-hub|plm-channel|plm@|chrome|context7|serena|notion|^plan|plan@|code@| plan | code | plm " || echo "  (none)"
	@echo "=== MCP ==="
	@claude mcp list 2>/dev/null | grep -iE "plm|plm-hub|plm-channel|chrome|context7|serena|notion|ouroboros" || echo "  (none)"
	@echo "=== ouroboros env (선택 — PLM-only면 불필요) ==="
	...
	@if [ -f "$(ENV_FILE)" ]; then \
		...
		for k in OUROBOROS_URL DEVELOPER_USER_ID PROJECT_ID; do \
			grep -q "^$$k=" $(ENV_FILE) && echo "    [OK] $$k" || echo "    [-] $$k 미설정(ouroboros 쓸 때만 필요)"; \
		done; \
	else \
