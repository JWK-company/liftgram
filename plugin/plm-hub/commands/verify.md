...
2. **문서 드리프트** (TRACKED = URS/UCS/SRS/SAD/ADR/Roadmap):
   - 로컬: `.ouroboros/docs/{requirements,design,decisions,roadmap}/*.json`(ADR-019 canonical, `.md` 레거시 병행) 파일명 stem(= 아티팩트 코드). `_`접두·`sync:false` 제외.
   - PLM: `/export?project=<p>`의 active(비-Replaced) TRACKED 아티팩트 코드.
...
4. **보고 + 정합(사용자 확인 후)**:
   ...
   - `plm_only` 문서 → `/plm-hub:pull`(PLM 편집분 회수) 또는 의도된 삭제면 PLM에서 Replaced 처리.
   - 소스에 없는 active Code → `/plm-hub:codescan`(GC가 리네임/삭제 Code를 Replaced 처리).
