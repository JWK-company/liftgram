...
원칙: **로컬 `.ouroboros/docs/*.md` = SSOT(본문·관계)**, **PLM = 동기 대상 + 게이트/추적/영향전파 권위**(Notion 대체).

## 데이터 흐름
```
 plan/code 스킬 ── 발급/편집 ──▶ .ouroboros/docs/{requirements,design,decisions,roadmap}/*.md   (SSOT)
                                          │  (PostToolUse Edit|Write)
                                          ▼  plm-sync.sh → POST /import (frontmatter+본문, owner 관계)
 ...
 소스코드 // @plm SRS-NNN ── /plm-hub:codescan ──▶ Code 아티팩트(loc:path:line)+realizes→SRS/SAD
                                          └─▶ 대상 .md frontmatter에 code_refs 역기재
 PLM/대시보드 편집 ── /plm-hub:pull ──▶ 로컬 .md 역반영(Status·관계·본문)
 Claude ◀── plm MCP 14도구(search·gates·artifact_*·relation_*·export/import…) ── 직접 조회/변경
```

## 딥링크 추적 (아티팩트 ↔ 실제 문서/코드)
- **문서 위치(결정적)**: 아티팩트 `<CODE>` → `.ouroboros/docs/{type→dir}/{CODE}.md`.
- **코드 역링크(소스 주석)**: 구현부에 `// @plm SRS-002` → `/plm-hub:codescan`이 Code 아티팩트(body `loc: \`path:line\``)+`realizes` 생성.
- **문서 역기재**: codescan이 대상 SRS/SAD `.md`에 `code_refs: [path:line]` 자동 기입.
...
| PostToolUse(Edit\|Write) | `plm-sync.sh` | 변경된 기획 `.md` 1건 즉시 PLM upsert(관계 포함) | 비차단·graceful |
...
{ "project": "<plm-project-id>", "api_url": "https://plm.shoi.ch" }
// .ouroboros/env/.env
...
| `plm-hub:sync` | 로컬 `.md` 전체를 PLM과 일괄 동기(import 중심) |
| `plm-hub:pull` | **PLM/대시보드 → 로컬 `.md` 역반영**(Status·관계·본문 회수) |
...
## plan/code 와의 관계
- **plan**: Notion hook(`notion-detect`) 제거 — PLM이 대체. 로컬 `gate-check`(frontmatter 즉시검증)는 오프라인 보조로 유지, `plm-gate`(DB 권위)가 주.
