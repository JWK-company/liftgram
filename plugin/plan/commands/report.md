---
description: HTML 보고서 작성(타입별 가이드 — governance·survey·analysis·verification·showcase·business, 3사이클 검증 후 제출)
argument-hint: "[타입] [보고서 주제/범위] — 타입 생략 시 governance(프로젝트 전체)"
---

# /report — 기획 HTML 보고서 (타입 라우팅)

## 0단계 — 타입 판별 · 가이드 로드 (필수 선행)

인자 첫 토큰(또는 요청 성격)으로 타입을 판별하고, **이 플러그인의 `templates/reports/_COMMON.md`(공통 규약 — 전달 방법·CSP sandbox 제약·3사이클 검증)와 해당 타입 가이드를 반드시 읽은 뒤** 작성한다:

| 타입 | 가이드 | 언제 |
|------|--------|------|
| `governance`(기본) | `templates/reports/governance.md` | 기획 현황·게이트·추적성 보고(아래 1~4단계) |
| `survey` | `templates/reports/survey.md` | 사용자에게 여러 결정을 받는 설문(예시답변+입력칸+응답 수집기) |
| `analysis` | `templates/reports/analysis.md` | 요구 전수 정리·정합 감사·설계 검토(누락 0·원문 보존) |
| `verification` | `templates/reports/verification.md` | E2E·수정 증명(증거·재현 커맨드) |
| `showcase` | `templates/reports/showcase.md` | 기능 소개·데모(화면 중심) |
| `business` | `templates/reports/business.md` | 시장·경쟁·수익화 조사 보고서(/business 산출 Business 아티팩트의 첨부 본체 — 예시 보고서 수준) |

산출: `.ouroboros/docs/report/YYYYMMDD_{slug}_{type}.html`. 전달은 _COMMON.md §2(기본=메신저 첨부 — `plm_upload` 후 message `files`/attachments).
비-governance 타입은 해당 가이드의 구조를 따르고 아래 1~2단계의 데이터 수집·구성 대신 가이드 내용을 적용한다(3단계 검증·4단계 제출은 전 타입 공통).

---

이하는 **governance 타입**의 상세 프로토콜.
...
**PLM에서 (거버넌스 사실):**
- `/export?project=<p>` — 전 아티팩트(URS/UCS/SRS/SAD/ADR/Roadmap) + status + owner 관계. active(비-Replaced)만 집계.
