Ouroboros 워크플로우 시스템 가이드를 보여줍니다. 인자에 따라 특정 주제를 설명합니다.
...
📦 구성 요소
  ...
  Template (이 프로젝트에 설치됨)
    ├─ 14개 스킬    — /spec, /execute, /fix, /qa 등
    ├─ 6개 Hook     — 자동 가이드, 범위 감시, 대화 동기화
    ├─ 12개 프로토콜 — Auto-collection, Signal, RRF 검색
...
📖 상세 가이드: /guide <주제>
  ...
  /guide config       — 설정 파일 (.ouroboros/env/.env, .mcp.json)
```
